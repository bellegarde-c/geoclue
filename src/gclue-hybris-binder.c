...
static void gclue_hybris_interface_init (GClueHybrisInterface * iface);
...
#define GNSS_NI_REMOTE(v)      GNSS_IFACE  (v, "IGnssNi")
#define GNSS_NI_CALLBACK(v)    GNSS_IFACE  (v, "IGnssNiCallback")
#define GNSS_XTRA_REMOTE(v)    GNSS_IFACE  (v, "IGnssXtra")
#define GNSS_XTRA_CALLBACK(v)  GNSS_IFACE  (v, "IGnssXtraCallback")
#define AGNSS_REMOTE(v)        GNSS_IFACE  (v, "IAGnss")
#define AGNSS_CALLBACK(v)      GNSS_IFACE  (v, "IAGnssCallback")
#define AGNSS_RIL_REMOTE(v)    GNSS_IFACE  (v, "IAGnssRil")
#define AGNSS_RIL_CALLBACK(v)  GNSS_IFACE  (v, "IAGnssRilCallback")
...
static gboolean
nmeaChecksumValid (GString *nmea)
{
        ...
        for (int i = 1; i < nmea->len; ++i) {
                if (nmea->str[i] == '*') {
                        ...
                        checksum ^=
                            g_ascii_strtoll (g_string_new_len
                                             (nmea->str + i + 1, 2)->str, NULL,
                                             16);
                              ...
                              void                 *user_data)
{
        ...
        if  (!g_strcmp0 (iface, GNSS_CALLBACK (priv->m_gnssVersion))) {
                ...
                case GNSS_LOCATION_CB:
                        {
                                GClueHybrisLocation *loc =
                                    g_slice_new0 (GClueHybrisLocation);

                                const GnssLocation *location =
                                    geoclue_binder_gnss_decode_struct
                                    (GnssLocation,
                                     &reader);

                                loc->timestamp = location->timestamp;

                                if (location->gnssLocationFlags &
                                ...
                                if (location->gnssLocationFlags &
                                    HYBRIS_GNSS_LOCATION_HAS_ALTITUDE)
                                        loc->altitude =
                                            location->altitudeMeters;

                                if (location->gnssLocationFlags &
                                    HYBRIS_GNSS_LOCATION_HAS_SPEED)
                                        loc->speed =
                                            location->speedMetersPerSec *
                                            MpsToKnots;

                                if (location->gnssLocationFlags &
                                    HYBRIS_GNSS_LOCATION_HAS_BEARING)
                                        loc->direction =
                                ...
                                if ((location->gnssLocationFlags &
                                    ...
                                    || (location->gnssLocationFlags &
                                        HYBRIS_GNSS_LOCATION_HAS_VERTICAL_ACCURACY))
                                {
                                        GClueHybrisAccuracy *accuracy =
                                            g_slice_new0 (GClueHybrisAccuracy);
                                        if (location->gnssLocationFlags &
                                            HYBRIS_GNSS_LOCATION_HAS_HORIZONTAL_ACCURACY)
                                        {
                                                accuracy->horizontal =
                                ...
                                g_signal_emit (hbinder, signals[SET_LOCATION],
                                               0, loc);
                        }
                        break;
                case GNSS_STATUS_CB:
                ...
                case GNSS_SV_STATUS_CB:
                        {
                                ...
                                for (int i = 0; i < svStatus->numSvs; ++i) {
                                        GClueHybrisSatelliteInfo *satInfo =
                                            g_slice_new0
                                            (GClueHybrisSatelliteInfo);
                                        GnssSvInfo svInfo =
                                            svStatus->gnssSvList[i];
                                        satInfo->snr = svInfo.cN0Dbhz;
                                        satInfo->elevation =
                                            svInfo.elevationDegrees;
                                        satInfo->azimuth =
                                            svInfo.azimuthDegrees;
                                        int prn = svInfo.svid;
                                        // From https://github.com/barbeau/gpstest
                                        // and https://github.com/mvglasow/satstat/wiki/NMEA-IDs
                                        if (svInfo.constellation == SBAS) {
                                                prn -= 87;
                                        } else if (svInfo.constellation ==
                                                   GLONASS) {
                                                prn += 64;
                                        } else if (svInfo.constellation ==
                                                   BEIDOU) {
                                        ...
                                        satInfo->prn = prn;
                                        satellites =
                                            g_list_append (satellites, satInfo);

                                        if (svInfo.svFlag &
                                            HYBRIS_GNSS_SV_FLAGS_USED_IN_FIX)
                                                usedPrns =
                                                    g_list_append (usedPrns,
                                                                   &prn);
                                }
                ...
                case GNSS_NMEA_CB:
                        {
                                gint64 timestamp;
                                if (gbinder_reader_read_int64
                                    (&reader, &timestamp)) {
                                        char *nmeaData =
                                            gbinder_reader_read_hidl_string
                                            (&reader);
                                        if (nmeaData) {
                                                processNmea (timestamp,
                                                             nmeaData);
                                                g_free (nmeaData);
                                        }
                                }
                ...
                case GNSS_SET_CAPABILITIES_CB:
                        {
                                ...
                                if (gbinder_reader_read_uint32
                                    (&reader, &capabilities)) {
                                        g_debug
                                            ("lcGeoclueHybris: capabilities %d",
                                             capabilities);
                                }
                        }
                        break;
                ...
                *status = GBINDER_STATUS_OK;
                return
                    ...
                    (gbinder_local_object_new_reply (obj), 0);
        } else {
...
GBinderLocalReply *geoclue_binder_agnss_callback (GBinderLocalObject *obj,
                                                  GBinderRemoteRequest *req,
                                                  guint code,
                                                  guint flags,
                                   ...
                                   void                 *user_data)
{
        ...
        if  (!g_strcmp0 (iface, AGNSS_RIL_CALLBACK (priv->m_gnssVersion))) {
                ...
                case AGNSS_RIL_REQUEST_REF_LOC_CB:
                        g_debug
                            ("lcGeoclueHybris: AGNSS RIL request ref location");
                        break;
                default:
                        g_warning ("Failed to decode callback %u", code);
                        break;
...
static void gclue_hybris_binder_class_init (GClueHybrisBinderClass *klass)
{
        signals[SET_LOCATION] =
            g_signal_lookup ("setLocation", GCLUE_TYPE_HYBRIS);
}
...
static void gclue_hybris_interface_init (GClueHybrisInterface *iface)
{
        ...
        iface->gnssXtraInjectXtraData =
            gclue_hybris_binder_gnssXtraInjectXtraData;
...
gclue_hybris_binder_dropGnss (GClueHybrisBinder *hbinder)
{
        ...
        if (priv->m_remoteGnss) {
                gbinder_remote_object_remove_handler (priv->m_remoteGnss,
                                                      priv->m_death_id);
...
static gboolean
isReplySuccess (GBinderRemoteReply *reply)
{
        GBinderReader reader;
        gint32 status;
        gboolean result;
...
static GBinderRemoteObject *
getExtensionObject (GBinderRemoteReply *reply)
{
        GBinderReader reader;
        gint32 status;
        gbinder_remote_reply_init_reader (reply, &reader);
...
static void
on_hybris_binder_destroyed (gpointer data, GObject *where_the_object_was)
{
...
gclue_hybris_binder_get_singleton (void)
{
        ...
        if (hbinder == NULL) {
                ...
                g_object_weak_ref (G_OBJECT (hbinder),
                                   on_hybris_binder_destroyed, &hbinder);
        } else
...
gclue_hybris_binder_gnssCleanup (GClueHybris *hybris)
{
        ...
        if (priv->m_clientGnss) {
                gbinder_client_transact (priv->m_clientGnss, GNSS_CLEANUP, 0,
                                         NULL, NULL, NULL, NULL);
        }
}
                                          ...
                                          HybrisGnssAidingData  aidingDataFlags)
{
        ...
        if (priv->m_clientGnss) {
                ...
                gbinder_client_transact (priv->m_clientGnss,
                                         GNSS_DELETE_AIDING_DATA, 0, req, NULL,
                                         NULL, NULL);
...
gclue_hybris_binder_gnssSetPositionMode (GClueHybris *hybris,
                                         HybrisGnssPositionMode mode,
                                         HybrisGnssPositionRecurrence
                                         recurrence, guint32 minIntervalMs,
                                         guint32 preferredAccuracyMeters,
                                         guint32 preferredTimeMs)
{
        GClueHybrisBinder *hbinder;
        ...
        if (!status) {
                ...
                if (priv->m_remoteGnssNi) {
                        ...
                        reply =
                            ...
                            (priv->m_clientGnssNi, GNSS_NI_SET_CALLBACK, req,
                             &status);
                        ...
                        if (!status) {
                                if (!gbinder_remote_reply_read_int32
                                    ...
                                    || status != 0) {
                                        g_warning
                                            ("Initialising GNSS NI interface failed %d",
                                             status);
                                }
                        }
                        gbinder_local_request_unref (req);
                                   ...
                                   HybrisGnssUserResponseType  userResponse)
{
        ...
        if (priv->m_clientGnssNi) {
                ...
                reply =
                    gbinder_client_transact_sync_reply (priv->m_clientGnssNi,
                                                        GNSS_NI_RESPOND, req,
                                                        &status);

                if (!status) {
                        if (!gbinder_remote_reply_read_int32 (reply, &status)
                            || status != 0) {
...
gclue_hybris_binder_gnssXtraInit (GClueHybris *hybris)
{
        ...
        if (!status) {
                ...
                if (priv->m_remoteGnssXtra) {
                        ...
                        req =
                            gbinder_client_new_request (priv->m_clientGnssXtra);
                        gbinder_local_request_append_local_object (req,
                                                                   priv->m_callbackGnssXtra);
                        reply =
                            gbinder_client_transact_sync_reply
                            (priv->m_clientGnssXtra, GNSS_XTRA_SET_CALLBACK,
                             req, &status);

                        if (status || !isReplySuccess (reply)) {
                                g_warning
                                    ("Initialising GNSS Xtra interface failed");
                        }
                        gbinder_local_request_unref (req);
                }
        }
                                            ...
                                            gchar       *xtraData)
{
        ...
        if (priv->m_clientGnssXtra) {
                ...
                reply =
                    gbinder_client_transact_sync_reply (priv->m_clientGnssXtra,
                                                        GNSS_XTRA_INJECT_XTRA_DATA,
                                                        req, &status);
