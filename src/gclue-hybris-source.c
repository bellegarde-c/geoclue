...
connect_to_service (GClueHybrisSource *source)
{
        ...
        g_signal_connect (G_OBJECT (source),
                          "notify::location",
                          G_CALLBACK (on_location_changed), priv->hybris);

        g_signal_connect (priv->hybris,
                          "setLocation", G_CALLBACK (on_set_location), source);
...
{
        GClueLocationSourceClass *source_class =
            GCLUE_LOCATION_SOURCE_CLASS (klass);
...
gclue_hybris_source_init (GClueHybrisSource *source)
{
        ...
        g_object_set (G_OBJECT (source), "available-accuracy-level", level,
                      NULL);
...
gclue_hybris_source_start (GClueLocationSource *source)
{
        ...
        base_class =
            GCLUE_LOCATION_SOURCE_CLASS (gclue_hybris_source_parent_class);
